...
the toolbar right edge closes this document. On Windows, documents retain the
native MDI window controls supplied by the operating system.
...
but the tab active when \emph{OK} is pressed determines which operation is
performed. Control values on all three tabs are remembered for later calls during
the application session; the first alignment defaults to Z and Min.
