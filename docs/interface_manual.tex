...
used to reach it; temporary obstacle bitmaps are discarded after a level has
been processed. This limits retained memory while preserving a complete route
back to the platform.

For an accepted tip, the route search starts at the tip cone's lower center
and follows reachable predecessors down to the platform. The resulting
centerline may bend at lattice levels. A continuously tapered polygonal pipe
is built along the route and joined to a cylindrical base. Contacts for which
no collision-free external route exists are left for the future internal
pillar stage and do not emit incomplete geometry.
