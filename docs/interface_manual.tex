...
optimization work.
The \emph{Stop} toolbar button requests cooperative cancellation. Optimization
and support-generation workers stop between slicing or generation operations,
and the controls return to their idle state after all workers have exited. The
same button cancels an interactive slice calculation currently running in the
background.
...
Each contact tip starts at its contact point and follows the downward-oriented
normal of the nearest model surface. A two-dimensional bounding-volume index
accelerates surface lookup. The cone slope is calculated once from its radii
and height. If the surface normal is too shallow, the tip axis is tilted
downward until its angle from the build plane equals the critical angle minus
the cone slope angle. The cone sides and both polygonal end surfaces are
triangulated and combined in the generated support model.
