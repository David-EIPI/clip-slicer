...
The \emph{Supports} toolbar button or \emph{Slice > Highlight unsupported
areas} command analyzes the selected models in their transformed positions.
The analysis uses 0.1 model-unit layers and the configured critical support
angle and overhang coefficient. It runs in the background so that document
viewing remains responsive.

When analysis finishes, unsupported portions of layer surfaces are overlaid in
bright red-orange. The first layer is treated as fully supported. On each
...
in the form \texttt{Run 1 of 10 (S=12.34)}. Model transformations or
analysis-setting changes cancel stale optimization work.
The \emph{Stop} toolbar button requests cooperative cancellation. Optimization
workers stop between slicing layers, and the controls return to their idle
state after all workers have exited. The same button cancels an interactive
slice calculation currently running in the background.
...
The \emph{Slicing} tab contains the contour healing threshold. After initial
...
model and defaults to 10. The worker-thread setting controls the maximum
number of those attempts evaluated concurrently and defaults to 4.
