...
float f32le(const unsigned char* p) {
    ...
    if (!std::isfinite(value)) throw std::runtime_error("Binary STL contains a non-finite coordinate");
    return value;
}
...
TriangleMesh BinaryStlReader::read(const std::filesystem::path& path) const {
    ...
    if (!input) throw std::runtime_error("Cannot open STL file: " + path.string());
    return read(input);
}
...
TriangleMesh BinaryStlReader::read(std::istream& input) const {
    ...
    readExact(input, reinterpret_cast<char*>(countBytes.data()), countBytes.size(), "triangle count");
    ...
    for (std::uint32_t i = 0; i < count; ++i) {
        ...
        for (std::size_t v = 0; v < 3; ++v) {
            ...
            triangle.vertices[v] = {f32le(&record[offset]), f32le(&record[offset + 4]),
                                    f32le(&record[offset + 8])};
        }
